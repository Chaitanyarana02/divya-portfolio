...
export default function Navbar() {
  ...
  useEffect(() => {
    const path = location.pathname.split('/')[1]; // get first part of the path
  ...
  return (
    <>
      <motion.div
        ...
        className={`cursor-pointer text-5xl font-[400] absolute top-8 left-8 ${themeActive ? 'text-white' : ' text-[#181C14]'} uppercase glitchy-text`}
      >
        DB
      </motion.div>
      ...
      className="fixed top-5 right-5 z-50">
        ...
        >
          <CiMenuBurger className={`text-3xl ${themeActive ? 'text-white' : ' text-[#181C14]'}   `} />
        </button>
      </motion.div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <>
            ...
            <motion.div
              className="fixed top-0 right-0 w-64 h-full bg-white text-black z-50 shadow-xl px-6 py-4 flex flex-col gap-4"
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
            ...
            >
              <div className="flex justify-end absolute top-4 right-4">
                <button onClick={closeSidebar}>
                  <RxCross1 className="text-2xl hover:text-red-500 transition" />
                </button>
              </div>

              <h1
                style={{ fontFamily: "FontFinal" }}
                className="text-5xl font-[400] text-[#181C14] uppercase mb-4"
              >
                DB
              </h1>
